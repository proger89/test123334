...
Разбор сначала показывает выполненные пункты, затем пункты для повторения и подходящие короткие упражнения. Оценка по навыкам и подробные последствия открываются отдельно. Если игрок пытается запустить второе упражнение при незавершённом первом, появляется окно с объяснением и кнопкой «Продолжить прохождение». Закрытие окна сохраняет текущую страницу; начатое упражнение не теряется.
...
«Редактор» → код доступа → выбор исходной версии → «Создать черновик» → изменение шагов и действий → сохранение → проверка ошибок → пробное прохождение → публикация для обучения. Неопубликованный черновик видит только его автор. Проба не влияет на результаты сотрудников. После публикации новые учебные смены используют новую версию, а проверки и начатые смены сохраняют прежнюю.
